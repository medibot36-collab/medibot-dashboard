...
function watchActivePatient() {
  db.ref('Commands/activePatient').on('value', snap => {
    ...
    // If cleared or no patient set, show awaiting skeleton and stop
    if (!pid || pid === 'none') {
      activePatientId = '';
...
// ── Delete a patient and all their Firebase data ─────────────────
async function deletePatient(patientId) {
  ...
  try {
    ...
    // If the deleted patient was active, clear the dashboard card + Firebase
...
// ── Clear active patient from dashboard + Firebase (persists on refresh) ──
function clearPatientDisplay() {
  // Write 'none' to Firebase so the card stays cleared after refresh
