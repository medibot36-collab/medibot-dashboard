...
let activePatientId     = 'p1';
let notificationsEnabled = false;

// Vital timestamps
let lastHrUpdate   = 0;
let lastSpo2Update = 0;
let tsInterval     = null;
...
function watchActivePatient() {
  db.ref('Commands/activePatient').on('value', snap => {
    ...
    if (!pid || pid === 'none') {
      ...
      const fsEl = document.getElementById('fsActiveLabel');
    ...
    const fsEl = document.getElementById('fsActiveLabel');
    if (fsEl) fsEl.textContent = pid.toUpperCase();
  });
}
...
function handleVitals(snap) {
  ...
  if (hr > 0) {
    ...
    const tsEl = document.getElementById('hrTimestamp');
    if (tsEl) tsEl.style.display = 'flex';
  ...
  if (spo2 > 0) {
    ...
    const tsEl = document.getElementById('spo2Timestamp');
    if (tsEl) tsEl.style.display = 'flex';
    lastSpo2Update = Date.now();
  }
...
async function deletePatient(patientId) {
  ...
  try {
    ...
    if (patientId === activePatientId) clearPatientDisplay();
  } catch (err) {
...
function clearPatientDisplay() {
  if (db) db.ref('Commands/activePatient').set('none');
  document.getElementById('patientAwaiting').style.display      = 'flex';
  document.getElementById('patientActive').style.display         = 'none';
  document.getElementById('patientOfflineStatus').style.display  = 'none';
  ['piName','piID','piAge','piGender','piEmail','piDate']
    .forEach(id => { const el = document.getElementById(id); if (el) el.textContent = '—'; });
  updateRiskBorders('normal');
}
...
function updateAIRisk(hr, spo2) {
  ...
  updateRiskBorders(riskLevel);
  updateHealthSummary(hr, spo2, riskLevel);
}
...
function updateRiskMeter(score) {
  ...
  fill.style.width = s + '%';
  if (s > 66) {
    fill.style.background = 'linear-gradient(90deg, var(--accent) 0%, var(--warn) 40%, var(--danger) 100%)';
  } else if (s > 33) {
    fill.style.background = 'linear-gradient(90deg, var(--accent) 0%, var(--warn) 100%)';
  } else {
    fill.style.background = 'var(--accent)';
  }
}

function updateRiskBorders(level) {
  const targets = ['patientCard', 'aiCard', 'hrCard', 'spo2Card'];
  const classes = ['risk-border-safe', 'risk-border-moderate', 'risk-border-critical'];
  const map     = { normal: 'risk-border-safe', warning: 'risk-border-moderate', danger: 'risk-border-critical' };
  targets.forEach(id => {
    const el = document.getElementById(id);
    if (!el) return;
    el.classList.remove(...classes);
    if (map[level]) el.classList.add(map[level]);
  });
}

function updateHealthSummary(hr, spo2, level) {
  const el   = document.getElementById('healthSummary');
  const text = document.getElementById('healthSummaryText');
  if (!el || !text) return;

  let icon = '🩺', msg = '';
  const highHR  = hr   > HR_THRESHOLD;
  const lowSpo2 = spo2 < SPO2_THRESHOLD;

  if (level === 'danger') {
    if (highHR && lowSpo2) {
      msg = `Critical: elevated HR (${hr} bpm) and low SpO₂ (${spo2}%). Immediate attention required.`;
      icon = '🚨';
    } else if (highHR) {
      msg = `Elevated heart rate detected (${hr} bpm). HR exceeds safe threshold.`;
      icon = '⚠️';
    } else if (lowSpo2) {
      msg = `Oxygen saturation below normal (${spo2}%). SpO₂ intervention may be needed.`;
      icon = '💨';
    } else {
      msg = `Significant vital deviation detected. Z-score anomaly — monitor closely.`;
      icon = '🔴';
    }
  } else if (level === 'warning') {
    msg = `Mild deviation noted. HR: ${hr} bpm, SpO₂: ${spo2}%. Continue monitoring.`;
    icon = '🟡';
  } else {
    msg = `Patient vitals are stable. HR: ${hr} bpm, SpO₂: ${spo2}%. No anomalies detected.`;
    icon = '✅';
  }

  el.className   = `health-summary ${level === 'danger' ? 'danger' : level === 'warning' ? 'warning' : 'normal'}`;
  document.querySelector('#healthSummary .health-summary-icon').textContent = icon;
...
function startTimestampTicker() {
  ...
  tsInterval = setInterval(() => {
    const now = Date.now();
    updateVitalTimestamp('hrTimestamp',   'hrTs',   lastHrUpdate,   now);
    updateVitalTimestamp('spo2Timestamp', 'spo2Ts', lastSpo2Update, now);
  }, 1000);
}

function updateVitalTimestamp(containerId, textId, lastUpdate, now) {
  const container = document.getElementById(containerId);
  const textEl    = document.getElementById(textId);
  if (!container || !textEl || lastUpdate === 0) return;

  const sec = Math.floor((now - lastUpdate) / 1000);
  let label, cls;

  if (sec < 5)        { label = 'Updated just now';         cls = 'fresh'; }
  else if (sec < 30)  { label = `Updated ${sec}s ago`;       cls = 'fresh'; }
  else if (sec < 120) { label = `Updated ${sec}s ago`;       cls = 'stale'; }
  else                { label = 'Data may be outdated';       cls = 'dead';  }

  textEl.textContent    = label;
  container.className   = `vital-timestamp ${cls}`;
}

function toggleTheme(checked) {
  const theme = checked ? 'light' : 'dark';
  document.documentElement.setAttribute('data-theme', theme);
  localStorage.setItem('medbot_theme', theme);
  updateChartColors(theme);
}

function applyStoredTheme() {
  const saved   = localStorage.getItem('medbot_theme') || 'dark';
...
function updateChartColors(theme) {
  ...
  const tickColor = theme === 'light' ? '#5868a0' : '#8090b8';
  [hrChartInstance, spo2ChartInstance].forEach(chart => {
    if (!chart) return;
...
let activeJoystickPrefix = '';

function joystickStart(e) {
  e.preventDefault();
  joystickActive = true;
  activeJoystickPrefix = '';
...
function fsJoystickStart(e) {
  e.preventDefault();
  joystickActive = true;
  activeJoystickPrefix = 'fs';
  document.addEventListener('mousemove', joystickMove);
  document.addEventListener('mouseup',   joystickEnd);
  joystickMove(e);
}

function fsJoystickTouchStart(e) {
  e.preventDefault();
  joystickActive = true;
  activeJoystickPrefix = 'fs';
  document.addEventListener('touchmove', joystickTouchMove, { passive: false });
...
function joystickProcess(clientX, clientY) {
  const pfx    = activeJoystickPrefix;
  const outerId = pfx ? 'fsJoystickOuter' : 'joystickOuter';
  const knobId  = pfx ? 'fsJoystickKnob'  : 'joystickKnob';
  ...
  const pre    = pfx ? 'fsJ' : 'j';
  [pre+'Top', pre+'Bot', pre+'Lft', pre+'Rgt'].forEach(id =>
    document.getElementById(id)?.classList.remove('active'));
  const zoneMap = { forward: pre+'Top', backward: pre+'Bot', left: pre+'Lft', right: pre+'Rgt' };
...
function resetJoystick() {
  ['joystickKnob','fsJoystickKnob'].forEach(id => {
    const el = document.getElementById(id);
    if (el) { el.style.transform = 'translate(-50%, -50%)'; el.classList.remove('active'); }
  });
  ['jTop','jBot','jLft','jRgt','fsJTop','fsJBot','fsJLft','fsJRgt'].forEach(id =>
    document.getElementById(id)?.classList.remove('active'));
  stopRobot();
}
...
function sendRobotCommand(direction) {
  ...
  if (db) db.ref('Commands/robot').update({ direction, speed, timestamp: Date.now() });
  updatePathIndicator(direction);
}

function updatePathIndicator(direction) {
  const iconMap = {
    forward:  'fa-arrow-up',
  ...
  const icon   = iconMap[direction] || 'fa-minus';
  const label  = labelMap[direction] || 'STOPPED';
  const lastTs = new Date().toLocaleTimeString();

  ['pathArrow','fsPathArrow'].forEach(id => {
    const el = document.getElementById(id);
    if (!el) return;
    el.className = 'path-arrow' + (moving ? ' moving' : '');
    const i = el.querySelector('i');
    if (i) i.className = `fas ${icon}`;
  });
  ['pathDirText','fsPathDirText'].forEach(id => {
    const el = document.getElementById(id);
    if (el) el.textContent = label;
  });
  ['pathStatusDot','fsPathDot'].forEach(id => {
    const el = document.getElementById(id);
    if (el) el.className = 'path-status-dot' + (moving ? ' moving' : '');
  });
  const lastCmd = document.getElementById('pathLastCmd');
  if (lastCmd) lastCmd.textContent = `Last: ${label} · ${lastTs}`;
}
...
function initKeyboard() {
  ...
  document.addEventListener('keydown', e => {
    ...
    if (wasdEnabled && wasdMap[e.key] !== undefined) {
      e.preventDefault();
      const dir = wasdMap[e.key];
      if (dir === kbLastCmd) return;
      kbLastCmd = dir;
      if (kbThrottle) clearTimeout(kbThrottle);
      kbThrottle = setTimeout(() => { kbLastCmd = ''; }, 120);
      currentDirection = dir;
      sendRobotCommand(dir);
    }
  });
  ...
  document.addEventListener('keyup', e => {
    ...
    if (wasdEnabled && wasdMap[e.key] !== undefined && e.key !== ' ') stopRobot();
  });
}
...
function toggleKeyboardControl() {
  ...
  if (btn) btn.classList.toggle('kb-on', wasdEnabled);
  if (!wasdEnabled) stopRobot();
}
...
function initSpeedSlider() {
  document.getElementById('speedSlider').addEventListener('input', function () {
    ...
    const fsSlider = document.getElementById('fsSpeedSlider');
    const fsVal    = document.getElementById('fsSpeedVal');
    if (fsSlider) fsSlider.value = this.value;
    if (fsVal)    fsVal.textContent = this.value;
    if (currentDirection !== 'stop' && db) {
      db.ref('Commands/robot/speed').set(Number(this.value));
    }
  });
}

function syncFsSpeed(val) {
  document.getElementById('speedVal').textContent    = val;
  document.getElementById('speedSlider').value       = val;
  document.getElementById('fsSpeedVal').textContent  = val;
  if (currentDirection !== 'stop' && db) {
    db.ref('Commands/robot/speed').set(Number(val));
  }
}
